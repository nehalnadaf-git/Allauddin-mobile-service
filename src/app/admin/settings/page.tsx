...
export default function SettingsPage() {
    ...
    const inputCls = "w-full px-4 py-2.5 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.06)] focus:border-[#A78BFA] focus:ring-2 focus:ring-primary/20 outline-none text-sm font-dm";

    return (
        <div className="max-w-3xl">
            <h1 className="font-poppins font-bold text-2xl text-white mb-6">Settings</h1>

            {/* Shop Info */}
            <div className="bg-[rgba(255,255,255,0.03)] rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.2)] border border-[rgba(255,255,255,0.08)] backdrop-blur-xl p-6 mb-6">
                <h2 className="font-poppins font-semibold text-base text-white mb-4">Shop Information</h2>
                ...
                <button onClick={saveShop} disabled={saving} className="mt-4 flex items-center gap-2 px-6 py-2.5 bg-primary hover:hover:scale-[1.02] active:scale-[0.98] text-white text-sm font-poppins font-semibold rounded-xl transition-all">
            ...
            <div className="bg-[rgba(255,255,255,0.03)] rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.2)] border border-[rgba(255,255,255,0.08)] backdrop-blur-xl p-6 mb-6">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="font-poppins font-semibold text-base text-white">Social Media Links</h2>
                ...
                <div className="space-y-3">
                    {social.map((link, i) => (
                        <div key={i} className="flex items-center gap-2">
                            <select value={link.platform} onChange={e => { const n = [...social]; n[i] = { ...n[i], platform: e.target.value }; setSocial(n); }} className="px-3 py-2 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.06)] text-sm font-dm w-36">
                                {["WhatsApp", "Instagram", "Facebook", "YouTube", "Twitter/X", "Custom"].map(p => <option key={p}>{p}</option>)}
                            </select>
                            <input value={link.url} onChange={e => { const n = [...social]; n[i] = { ...n[i], url: e.target.value }; setSocial(n); }} placeholder="URL" className="flex-1 px-3 py-2 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.06)] text-sm font-dm" />
                            <button onClick={() => { const n = [...social]; n[i] = { ...n[i], isVisible: !n[i].isVisible }; setSocial(n); }} className="p-1.5 text-white/60">{link.isVisible ? <Eye size={14} /> : <EyeOff size={14} />}</button>
                            <button onClick={() => removeSocial(i)} className="p-1.5 text-white/60 hover:text-error"><Trash2 size={14} /></button>
                        </div>
                    ))}
                </div>
                <button onClick={saveSocial} className="mt-4 flex items-center gap-2 px-6 py-2.5 bg-primary hover:hover:scale-[1.02] active:scale-[0.98] text-white text-sm font-poppins font-semibold rounded-xl"><Save size={16} />Save Links</button>
            </div>



            {/* Password Change */}
            <div className="bg-[rgba(255,255,255,0.03)] rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.2)] border border-[rgba(255,255,255,0.08)] backdrop-blur-xl p-6">
                <h2 className="font-poppins font-semibold text-base text-white mb-4">Change Password</h2>
                ...
                <button onClick={doChangePassword} disabled={!curPwd || !newPwd} className="mt-4 px-6 py-2.5 bg-primary hover:hover:scale-[1.02] active:scale-[0.98] disabled:bg-border-grey text-white text-sm font-poppins font-semibold rounded-xl">Change Password</button>
            </div>
        </div>
    );
}
