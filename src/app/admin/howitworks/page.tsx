    ...
    return (
        <div className="p-6 max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                <h1 className="font-poppins font-bold text-2xl text-deep-text mb-2">How It Works</h1>
                <p className="font-dm text-muted text-sm mb-8">Edit the 3 steps shown in the How It Works section</p>

                <div className="space-y-4">
                    {steps?.map((step: any, index: number) => {
                        const form = edits[step._id] ? { ...step, ...edits[step._id] } : step;
                        return (
                            <div key={step._id} className="bg-white rounded-2xl p-5 card-shadow">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-3">
                                    ...
                                    <div>
                                        <label className="block text-xs font-poppins font-medium text-muted uppercase tracking-wide mb-1">Title</label>
                                        <input
                                            type="text"
                                            value={form.title}
                                ...
                                <div className="mb-4">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full px-3 py-2.5 rounded-xl border border-border-grey font-dm text-sm outline-none focus:border-primary transition-all resize-none"
                                    />
                                </div>

                                <button
                                    onClick={() => handleSave(step)}
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-poppins font-semibold transition-all ${saved === step._id ? "bg-success text-white" : "bg-primary hover:bg-hover-blue text-white"}`}
                                >
                                    <Save size={14} />
