    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                <h1 className="font-poppins font-bold text-2xl text-deep-text">Portfolio</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(items as any[])?.map((item: any) => (
                    <div key={item._id} className={`bg-white rounded-xl card-shadow overflow-hidden ${!item.isVisible ? "opacity-60" : ""}`}>
                        <div className="grid grid-cols-2 gap-0">
                            <div className="bg-light-grey aspect-[4/3] flex items-center justify-center text-xs text-muted font-dm relative">
                        ...
                        <div className="p-4">
                            <p className="font-poppins font-semibold text-sm text-deep-text mb-1">{item.title}</p>
                            <p className="font-dm text-xs text-muted line-clamp-2">{item.description}</p>
                            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border-grey">
                                <button onClick={() => toggleVisibility({ id: item._id })} className="p-1.5 text-muted hover:text-deep-text transition-colors">
                                    {item.isVisible ? <Eye size={14} /> : <EyeOff size={14} />}
                                </button>
                                <div className="flex-1" />
                                <button onClick={() => openEdit(item)} className="p-1.5 text-muted hover:text-primary transition-colors"><Edit2 size={14} /></button>
                                <button onClick={() => setDeleteConfirm(item._id)} className="p-1.5 text-muted hover:text-error transition-colors"><Trash2 size={14} /></button>
                            </div>
                        </div>
                    </div>
                ))}
            </div>
            {(!items || items.length === 0) && <p className="text-center py-12 text-muted font-dm text-sm">No portfolio items</p>}

            {/* Modal */}
            <Modal isOpen={isModalOpen} onClose={() => setModalOpen(false)} title={editingId ? "Edit Portfolio" : "Add Portfolio Work"}>
                <div className="space-y-3">
                    <div>
                        <label className="block font-dm text-sm font-medium text-deep-text mb-1">Title *</label>
                        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-border-grey bg-light-grey focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-sm font-dm" />
                    </div>
                    <div>
                        <label className="block font-dm text-sm font-medium text-deep-text mb-1">Description *</label>
                        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full px-4 py-2.5 rounded-xl border border-border-grey bg-light-grey focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-sm font-dm resize-none" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="block font-dm text-sm font-medium text-deep-text mb-1">Before Image</label>
                            <label className="flex items-center justify-center gap-1 w-full py-2.5 border-2 border-dashed border-border-grey rounded-xl cursor-pointer hover:border-primary text-xs text-muted font-dm">
                        ...
                        <div>
                            <label className="block font-dm text-sm font-medium text-deep-text mb-1">After Image</label>
                            <label className="flex items-center justify-center gap-1 w-full py-2.5 border-2 border-dashed border-border-grey rounded-xl cursor-pointer hover:border-primary text-xs text-muted font-dm">
            ...
            <Modal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Delete Portfolio Item">
                <p className="font-dm text-sm text-deep-text mb-4">Permanently delete this portfolio item and its images?</p>
