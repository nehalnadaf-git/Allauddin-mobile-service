    ...
    return (
        <div className="p-6 max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                <h1 className="font-poppins font-bold text-2xl text-white mb-2">Trust Banner Strip</h1>
                <p className="font-dm text-white/60 text-sm mb-8">Edit the 4 trust indicators shown below the hero section</p>
                ...
                <div className="space-y-4">
                    {trustItems?.map((item: any) => {
                        ...
                        return (
                            <div key={item._id} className="bg-[rgba(255,255,255,0.03)] rounded-2xl p-5 shadow-[0_4px_24px_rgba(0,0,0,0.2)] border border-[rgba(255,255,255,0.08)] backdrop-blur-xl">
                                <div className="flex items-center justify-between mb-4">
                                    <span className="font-poppins font-semibold text-sm text-white">Trust Item {item.displayOrder + 1}</span>
                                    <button
                                        onClick={() => toggleItem({ id: item._id, isVisible: !item.isVisible })}
                                        className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-poppins font-semibold transition-all ${item.isVisible ? "bg-success/10 text-success" : "bg-muted/10 text-white/60"}`}
                                    >
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-3">
                                    ...
                                    <div>
                                        <label className="block text-xs font-poppins font-medium text-white/60 uppercase tracking-wide mb-1">Label</label>
                                        <input
                                            type="text"
                                            value={form.label}
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-poppins font-semibold transition-all ${saved === item._id ? "bg-success text-white" : "bg-primary hover:hover:scale-[1.02] active:scale-[0.98] text-white"}`}
                                >
                                    <Save size={14} />
