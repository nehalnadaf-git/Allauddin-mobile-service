    ...
    return (
        <div className="bg-white rounded-2xl p-6 card-shadow mb-6">
            <div className="flex items-center justify-between mb-5">
                <h3 className="font-poppins font-semibold text-lg text-deep-text">{label}</h3>
                <button
                    onClick={() => toggleVisibility({ type, isVisible: !data?.isVisible })}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm font-poppins font-medium transition-all ${data?.isVisible ? "bg-success/10 text-success" : "bg-muted/10 text-muted"}`}
                >
            ...
            <div className="space-y-4">
                {fields.map(field => (
                    <div key={field.key}>
                        <label className="block text-xs font-poppins font-medium text-muted uppercase tracking-wide mb-1.5">
                            {field.label}
                        </label>
                        {field.type === "textarea" ? (
                            <textarea
                                ...
                                className="w-full px-3 py-2.5 rounded-xl border border-border-grey font-dm text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all resize-none"
                            />
                        ) : field.type === "select" ? (
                            <select
                                value={data?.[field.key] ?? ""}
                            ...
                            <input
                                ...
                                value={data?.[field.key] ?? ""}
                                onChange={e => setForm({ ...(data || {}), [field.key]: e.target.value })}
                                className="w-full px-3 py-2.5 rounded-xl border border-border-grey font-dm text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
                            />
                        )}
                    </div>
                ))}
            ...
            <button
                onClick={handleSave}
                className={`mt-5 flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-poppins font-semibold transition-all ${saved ? "bg-success text-white" : "bg-primary hover:bg-hover-blue text-white"}`}
            >
                <Save size={15} />
    ...
    return (
        <div className="p-6 max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                <h1 className="font-poppins font-bold text-2xl text-deep-text mb-2">Banner Management</h1>
                <p className="font-dm text-muted text-sm mb-8">Edit and toggle visibility of all banners across the site</p>
