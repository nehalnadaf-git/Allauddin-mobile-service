...
export default function ServicesAdminPage() {
    ...
    return (<div>
        <div className="flex items-center justify-between mb-6">
            <div>
                <h1 className="font-poppins font-bold text-2xl text-white">Services</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(services as any[])?.map((s: any) => (<div key={s._id} className={`bg-[rgba(255,255,255,0.03)] rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.2)] border border-[rgba(255,255,255,0.08)] backdrop-blur-xl p-5 ${!s.isVisible ? "opacity-60" : ""}`}>
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1"><p className="font-poppins font-semibold text-sm text-white">{s.name}</p><p className="font-dm text-xs text-white/60 line-clamp-2 mt-0.5">{s.description}</p><p className="font-poppins font-bold text-primary text-sm mt-2">From ₹{s.startingPrice}</p></div>
                </div>
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[rgba(255,255,255,0.08)]">
                    <span className="text-[10px] text-white/60 font-dm bg-[rgba(255,255,255,0.06)] px-2 py-0.5 rounded-full">{s.iconName}</span><div className="flex-1" />
                    <button onClick={() => toggle({ id: s._id })} className="p-1.5 text-white/60 hover:text-white">{s.isVisible ? <Eye size={14} /> : <EyeOff size={14} />}</button>
                    <button onClick={() => openEdit(s)} className="p-1.5 text-white/60 hover:text-primary"><Edit2 size={14} /></button>
                    <button onClick={() => setDel(s._id)} className="p-1.5 text-white/60 hover:text-error"><Trash2 size={14} /></button>
                </div>
            </div>))}
        </div>
        {(!services || !services.length) && <p className="text-center py-12 text-white/60 font-dm text-sm">No services</p>}
        <Modal isOpen={modal} onClose={() => setModal(false)} title={editId ? "Edit Service" : "Add Service"}>
            <div className="space-y-3">
                <div><label className="block font-dm text-sm font-medium mb-1">Name *</label><input value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.06)] focus:border-[#A78BFA] outline-none text-sm font-dm" /></div>
                <div><label className="block font-dm text-sm font-medium mb-1">Description *</label><textarea value={desc} onChange={e => setDesc(e.target.value)} rows={2} className="w-full px-4 py-2.5 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.06)] focus:border-[#A78BFA] outline-none text-sm font-dm resize-none" /></div>
                <div><label className="block font-dm text-sm font-medium mb-1">Starting Price (₹) *</label><input type="number" value={price} onChange={e => setPrice(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.06)] focus:border-[#A78BFA] outline-none text-sm font-dm" /></div>
                <div><label className="block font-dm text-sm font-medium mb-1">Icon</label><select value={icon} onChange={e => setIcon(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.06)] focus:border-[#A78BFA] outline-none text-sm font-dm">{icons.map(i => <option key={i} value={i}>{i}</option>)}</select></div>
                <button onClick={save} disabled={!name || !desc || !price} className="w-full py-3 bg-primary hover:hover:scale-[1.02] active:scale-[0.98] disabled:bg-border-grey text-white font-poppins font-semibold rounded-xl text-sm">{editId ? "Update" : "Create"}</button>
            </div>
        </Modal>
